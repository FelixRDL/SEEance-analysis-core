...
function ComponentProvider (options = {}) {
  ...
  async function pullRepo (repoUrl, token) {
    const targetPath = pathLib.join(that.componentsPath, makeFilenameSafe(repoUrl))
  ...
  /**
   * If an onlyUse option has been set, filter out unwanted components
   * @param {{
   *   name: string,
   *   path: string
   * }}component
  ...
  function filterComponents (component) {
    if (options.onlyLoad && options.onlyLoad.length > 0) {
      return options.onlyLoad.includes(component.name)
    } else {
      return true
    }
  }
  ...
  that.rebuild = async function () {
    ...
    await Promise.all(that.repositories.map(async (repoUrl) => {
      const path = await cloneRepo(repoUrl, options.token)
      const components = getAllComponentsOfRepository(path).filter(filterComponents)
  ...
  that.refresh = async function () {
    await Promise.all(that.repositories.map(async (repoUrl) => {
      const path = await pullRepo(repoUrl, options.token)
      const components = getAllComponentsOfRepository(path).filter(filterComponents)
